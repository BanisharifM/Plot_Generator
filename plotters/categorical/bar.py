...
class BarPlotter(BasePlotter):
   ...
   def get_colors(self, n_colors: int):
       """Get colors from config or generate defaults."""
       if self.config.color_palette and len(self.config.color_palette) > 0:
           return self.config.color_palette
       else:
           colors_array = plt.cm.tab10(np.linspace(0, 1, n_colors))
           return [mcolors.to_hex(c) for c in colors_array]
   ...
   def create_plot(self) -> Tuple[plt.Figure, plt.Axes]:
       ...
       if self.stacked:
           ...
           for i, y_col in enumerate(self.y_columns):
               ...
               if self.orientation == 'vertical':
                   ax.bar(x, self.data[y_col], self.bar_width, 
                         bottom=bottom, label=y_col, color=color)
                   bottom += self.data[y_col]
               else:
                   ax.barh(x, self.data[y_col], self.bar_width, 
                          left=bottom, label=y_col, color=color)
                   bottom += self.data[y_col]
           ...
           # Simple bar chart or auto-grouped for multiple columns
